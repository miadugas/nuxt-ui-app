<template>
  <div v-if="post" class="max-w-3xl mx-auto py-12">
    <h1 class="text-4xl font-bold mb-4">{{ post.title }}</h1>
    <img v-if="post.mainImage" :src="urlFor(post.mainImage).width(800).url()" :alt="post.title" class="w-full h-auto mb-6 rounded-lg" />
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center">
        <img v-if="post.authorImage" :src="post.authorImage" :alt="post.author" class="w-10 h-10 rounded-full mr-4" />
        <div>
      ...
      <p class="rounded-full bg-cyan-500/10 px-3 py-1 text-xs font-semibold leading-6 text-cyan-500 ring-1 ring-inset ring-cyan-500/20">{{
